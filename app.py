...
# Charge .env en local (sur Heroku utiliser Config Vars)
load_dotenv()

"""
Backend API-only (Heroku). Frontend déployé séparément (Vercel).
"""
...
# --- CORS ---
# Tu peux forcer via Config Var CORS_ORIGINS="https://nessbook.vercel.app,https://autre-domaine.com"
_env_origins = os.environ.get("CORS_ORIGINS")
if _env_origins:
    ORIGINS = [o.strip() for o in _env_origins.split(",") if o.strip()]
else:
    ORIGINS = ["https://nessbook.vercel.app"]  # défaut: ton front Vercel
...
CORS(
    ...
    resources={
        r"/api/*": {
            ...
            "supports_credentials": False  # mettre True seulement si tu utilises des cookies
        },
        r"/": {
            "origins": ORIGINS,
            "methods": ["GET", "OPTIONS"],
...
# Route catch-all pour préflights OPTIONS (robustesse supplémentaire)
